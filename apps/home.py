...
def prepro(df1):
	...
	top_screens=pd.read_csv('/content/drive/MyDrive/kuliah/dataset/top_screens.csv')
...
# 	top_screens=np.array(top_screens.loc[:,'top_screens'])
# 	for i in top_screens:
# 		df1[i]=df1.screen_list.str.contains(i).astype(int)
# 	for i in top_screens:
# 		df1['screen_list']=df1.screen_list.str.replace(i+',','')
# #menghapus double layar
# 	layar_loan = ['Loan','Loan2','Loan3','Loan4']
# 	df1['jumlah_loan']=df1[layar_loan].sum(axis=1)
# 	df1.drop(columns=layar_loan, inplace=True)

# 	layar_saving = ['Saving1','Saving2','Saving2Amount','Saving4','Saving5','Saving6','Saving7','Saving8','Saving9','Saving10']
# 	df1['jumlah_loan']=df1[layar_saving].sum(axis=1)
# 	df1.drop(columns=layar_saving, inplace=True)

# 	layar_credit = ['Credit1','Credit2','Credit3','Credit3Container','Credit3Dashboard']
# 	df1['jumlah_credit']=df1[layar_credit].sum(axis=1)
# 	df1.drop(columns=layar_credit, inplace=True)

# 	layar_cc = ['CC1','CC1Category','CC3']
# 	df1['jumlah_cc']=df1[layar_cc].sum(axis=1)
# 	df1.drop(columns=layar_cc, inplace=True)
# 	df1['lainnya']=df1.screen_list.str.count(',')
...
# 	df1_numerik=df1.drop(columns=['first_open','screen_list','enrolled_date','selisih'], inplace=False)
# 	df1_copy = df1_numerik.copy()
# 	df1_numerik=df1.drop(columns=['user'], inplace=False)
# 	df1_types = df1.dtypes.astype(str)
# 	st.write(df1_numerik.head(10))
# 	df1_types = df1.dtypes.astype(str)
# 	st.write(df1_types)
# #membuat korelasi matrik
# 	korelasi = df1_numerik.drop(columns=['enrolled'], inplace=False).corrwith(df1_numerik.enrolled)
# 	plot=korelasi.plot.bar(title='korelasi variabel')
# 	st.set_option('deprecation.showPyplotGlobalUse', False)
# 	st.pyplot()
